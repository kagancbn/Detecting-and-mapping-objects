...
#define minvote 35				//Min vote value in houghspace it can be im.w* im.h * 0.0004
#define number_of_lines 40		//Max number of detecting lines
...
{
	image out = make_empty_image(input.w, input.h, 1);
...
image CannyEdge(gradient grad)
{
	...
	image out = make_empty_image(HysWidth, HysHeight, 1);
...
void nonmax_houghspace(hough HoughSpace, int kernelSize)
{
	int width = (HoughSpace.w - kernelSize + 1);
	int height = (HoughSpace.h - kernelSize + 1);

	for (int row = 0; row < height; row += 7)
		for (int col = 0; col < width; col += 7) {
			int temp = 0;
			int buffer = 0;
			int temp_buff = 0;
			//Loop for find max in grid
			for (int krow = 0; krow < kernelSize; krow++)
				for (int kcol = 0; kcol < kernelSize; kcol++) {
					buffer = (row + krow) * HoughSpace.w + (col + kcol);
					if (HoughSpace.data[buffer] > temp)
					{
						temp = HoughSpace.data[buffer]; // find max val in grid
						temp_buff = buffer;
					}
			...
			if (temp != 0)
				for (int krow = 0; krow < kernelSize; krow++)
					for (int kcol = 0; kcol < kernelSize; kcol++) {
						...
						if (HoughSpace.data[buffer] < temp)
							HoughSpace.data[buffer] = 0; // surpress non max cells in grid
						if (HoughSpace.data[buffer] == temp && buffer != temp_buff)
							HoughSpace.data[buffer] = 0; // surpress non max cells in grid
					}
		}
}
void DrawSelectedLines(hough HoughSpace, image im, int& numberofline, line*& Line) {
	...
	nonmax_houghspace(HoughSpace, 41); /// kernerl size ----------------------------------------------------------------w
	...
	for (int i = 0; i < linecount; i++)
		for (int row = 4; row < im.h - 4; row++)
			for (int col = 4; col < im.w - 4; col++)
				if (Line[i].distance - HoughSpace.h / 2 == int(col * cos(Line[i].theta * PI / 180) + row * sin(Line[i].theta * PI / 180))) {
					bufpos = (row + 4) * psw + (col + 4) * im.c; // With smoothing kernel, gradient kernel, nonmax kernel, first and last 3 pixels are removed
...
void noktalariSirala(point points[], int n) {
	int i, j;
	point temp;
	...
	for (i = 0; i < n - 1; i++) {
		for (j = 0; j < n - i - 1; j++) {
			...
			else if (points[j].Y == points[j + 1].Y && points[j].X > points[j + 1].X) {
				temp = points[j];
				points[j] = points[j + 1];
				points[j + 1] = temp;
			}
...
void saveProjection(float* Projection)
{
	...
	c[0] = "Data/Projection.txt";
	std::ofstream file(c[0]);
	for (int i = 0; i < 3; i++)
	{
		file << Projection[i * 4] << " " << Projection[i * 4 + 1] << " " << Projection[i * 4 + 2] << " " << Projection[i * 4 + 3] << std::endl;
	}
}
void readProjection(float* Projection)
{
	...
	if (file.is_open()) {
		for (int i = 0; i < 3; i++)
		{
			file >> Projection[i * 4] >> Projection[i * 4 + 1] >> Projection[i * 4 + 2] >> Projection[i * 4 + 3];
		}
	}
}
